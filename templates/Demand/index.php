        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <div class="row d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">View Listed Demand</h1>
                            ...
                            <table border="3" cellspacing="0" cellpadding="3">
                                ...
                                foreach ($demand as $d): ?>
                                    <tr>
                                        <td><?= $i; ?></td>
                                        <?php if ($type == 'Buyer' || $type == 'Admin') {?>
                                        <td><?= $this->Html->link($d->quotation_number, ['controller' => 'quotation', 'action' => 'index', $d->id]) ?></td>
                                        <?php }?>
                                        <td><?= h($d->demand) ?></td>
                                        <td><?= h($d->amount) ?></td>
                                        <td><?= h($d->description) ?></td>
                                        <td><?= $this->Html->image($d->image1,['style'=>'height:100px']) ?></td>
                                        <td><?= h($d->business_name) ?></td>
                                        <td><?= h($d->business_num) ?></td>
                                        <td><?= h($d->business_email) ?></td>
                                        <td><?= h($d->business_address) ?></td>
                                        <td><?= h($d->city) ?></td>
                                        <td><?= h($d->state) ?></td>
                                        <td><?= h($d->postcode) ?></td>
                                        <td><?= h($d->ABN) ?></td>
                                        ...
                                        <td class="actions">
                                            <?= $this->Html->link(__('View'), ['action' => 'view', $d->id],['class'=>'fas fa-eye']) ?><br>
                                            <?php if ($type == 'Buyer' || $type == 'Admin') {?>
                                            <?= $this->Html->link(__('Edit'), ['action' => 'edit', $d->id],['class'=>'fas fa-edit']) ?><br>
                                            <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $d->id], ['class'=>'fas fa-trash','confirm' => __('Are you sure you want to delete {0}?', $d->demand)]) ?>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                    <?php $i++; endforeach; ?>
